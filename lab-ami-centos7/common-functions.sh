...
CheckSELinux() {
	...
	if [ "$STATUS" != 'disabled' ]; then 
		...
		info "  curl -s https://gitlab.com/thecloudcareers/opensource/-/raw/master/lab-ami-centos7/vminit.sh | sudo bash"
		exit 1
	fi
}
...
DownloadJava() {
	...
	case $1 in
		8) 
			curl -s https://gitlab.com/thecloudcareers/opensource/-/raw/master/lab-ami-centos7/java-params >/tmp/java-params
